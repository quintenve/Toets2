...
public interface SnackRepository extends CrudRepository<Snack, Integer> {
    ...
    @Query("select s from Snack s where (?1 is null or (s.price + s.priceSideDish <= ?1)) and (?2 is null or s.vegan = ?2)")
    List<Snack> findByFilterSideDishIncl(Double maxPrice, Boolean vegan);
}
